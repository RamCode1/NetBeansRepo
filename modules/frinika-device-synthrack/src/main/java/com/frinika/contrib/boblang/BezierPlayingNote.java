...
public class BezierPlayingNote {
    ...
    private int underSampling, // Undersampling rate
    ...
    // Various flags
    private boolean isReleased;                 // True if released

    // Links to other objects and classes
    private BezierSynth bezierSynth;                // Which generates sound sample values
    private BezierEnvelopeShaper shaper;                     // Envelope shaper
    ...
    /**
     * Set the release status for this note, recording the index number where
    ...
    /**
     * Get the next buffer of data from the sound buffer. This method performs
     * the undersampling necessary to convert from the internal sample rate (say
     * 196000) to the output rate (say 48000). This is achieved by incrementing
     * the index by the undersampling rate each time.
     */
     ...
     * the index by the undersampling rate each time.
     *
     * Adjustments for Frinika 32 bit floating point version startBuffer and
     * endBufferPos - for realtime parameter modifications Add (mix) new samples
     * to the buffer instead of replace.
     *
     * @author Bob Lang
