...
public class WaveSupport {
    ...
    public static final int PROP_COPY = 1, // Copy the wave form
    ...
    private static Graph[] graphList = new Graph[MAX_GRAPH_HANDLE];

    /**
     * Rescale the wave form so it uses the dynamic range specified by the given
     * amplitude.
     *
    ...
    /**
     * Method to propagate a waveform to fill a complete array. It uses the
     * possible propagation options.
     *
    ...
    /**
     * Method to apply a rectangular filter to a half wave and return the
     * resulting filtered output. The method needs to know how the half wave
     * will be propagated, hence the option parameter
     */
    ...
    /**
     * Method to change the wavelength waveform using linear interpolation. The
     * input wave is assumed to be a single cycle.
     */
    ...
    /**
     * Method to plot the data in as a FFT. The x co-ordinates are frequency
     * bins which are converted to the appropriate frequencies before being
     * plotted
     *
     * @param handle - graph window to create or re-use
